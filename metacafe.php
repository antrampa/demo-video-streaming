...
function GetDownloadLink($url)
{
	...
	// ------------------------------------------------------------------------------------------------
	return $YTVidURL; //$RetBuff;
}
		

function getMetacafeVideoID($url)
{
	//$url = "http://www.metacafe.com/watch/12345/blah_blah/";

	if(preg_match("/((http:\/\/)?(www\.)?metacafe\.com)(\/watch\/)(\d+)(.*)/i", $url, $match))
	{
    	//echo "VideoID:".$match[5];
		print_r($match);
		return $match[5];
	}	 	
	return "";
...
//echo "xaxaaxa:".download("http://www.videaki.com/index.php");
function download($url)
{
	//http://www.videaki.com/index.php	
	//my
	$new_flv_path = dirname(_FILE_).'/flvs/metacafe-'.time().'.flv' ;
