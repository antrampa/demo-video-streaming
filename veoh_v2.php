...
function getVeohVideoUrl($xmlString)
{
	...
	//echo "<br><br>fullPreviewHashPath:".$videoAttributes['fullPreviewHashPath'];
	
	
	$new_flv_path = dirname(_FILE_).'/flvs/veoh-'.time().'.flv' ;
	//$data = file_get_contents($flv_http_path);
	//echo "url: ".$url;
	$content = file_get_contents($videoAttributes['fullPreviewHashPath']);
...
function getVeohVideoId($url)
{
	$n = parse_url($url);
	$p = explode("/",$n['path']);
	//die($p[count($p)-1]);
	$video_id = $p[count($p)-1]; //"v19702969EXmXfwMY";
	//die($video_id);						 
	return $video_id;
}
