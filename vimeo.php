...
function vimeoid2url($id) 
{
	...
	//my
	$new_flv_path = dirname(_FILE_).'/flvs/vimeo-'.time().'.flv' ;
	//$data = file_get_contents($flv_http_path);
	//echo "url: ".$url;
	$content = file_get_contents($url);
	file_put_contents($new_flv_path, $content);

	return $new_flv_path; //$url;
}	

function getVimeoVideoId($url)
{
	$v = parse_url($url);		
	$video_id = str_replace("/","",$v['path']);
	return $video_id;
}
...
function downloadDailymotionFlvFile($dailymotionUrl)
{
	$content = file_get_contents($dailymotionUrl);
	...
	//print_r($headers);
	$header_str = str_replace('Location: ', '', $headers[8]);
	$flv = file_get_contents(str_replace('Location: ', '', $headers[8]));

	$file = "flvs/dailymotion-".time().".flv";
