...
function GrabFlvFromYoutube_v2( $url )
{
 ...
 $vout="";
    if(isset($url))
        ...
        if (strlen($url)>0)
            {
            if (preg_match("/v=([^(\&|$)]*)/", trim($url),$matches))
                {
                $VideoID = trim($matches[1]);
                }
            elseif (preg_match("/^([^(\&|$)]{10,12})$/",trim($url),$matches))
                {
                $VideoID = trim($matches[1]);
                }
            else
		...
		//$flv_http_path=$RetBuff.$vout;
		$flv_http_path=$VideoURL; 

 //echo "<br>flv_http_path:".$flv_http_path;

 set_time_limit(0);

 $data = file_get_contents($flv_http_path);

 $new_flv_path = dirname(_FILE_).'/flvs/youtube'.time().'.flv' ;
